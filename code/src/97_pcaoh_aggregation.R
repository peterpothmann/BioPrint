## ---------------------------
##
## Script name: 97_caoh_aggregation.R
##
...
submit <- tibble(paths = list.files(paste0(workPath, "00_data/03_stage/pcaoh/tif"), full.names = T)) %>%
  rowwise() %>%
...
# write files
write_tsv(submit, paste0(slmPath, "pcaoh-aggregation-setup.txt"), col_names = TRUE)

# create a submit file as the set up file is to long for submiting
submitLen <- unique(submit$submitID)
write_tsv(tibble(submitLen), paste0(slmPath, "pcaoh-aggregation-slurm.txt"), col_names = FALSE)
